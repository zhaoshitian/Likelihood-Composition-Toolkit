    ...
    def forward_debias_batched(self, args, x):
        ...
        if data_type[0] == 'image':
            if args.noise_step is None:
                image_batch = [Image.open(open(dp, "rb")).convert("RGB") for dp in data_path_batch]

                # if getattr(self.model.config, "image_aspect_ratio", None) == "pad":
                #     image_batch = [expand2square(image, tuple(int(x * 255) for x in self.image_processor.image_mean)) for image in image_batch]
                
                image_batch = [self.image_processor.preprocess(image, return_tensors="pt")["pixel_values"] for image in image_batch]
            else:
